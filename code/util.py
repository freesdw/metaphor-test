...
def evaluate(idx2pos, evaluation_dataloader, model, criterion, using_GPU):
    ...
    attn_acc = []
    for (eval_pos_seqs, eval_text, eval_lengths, eval_labels, eval_para_idx,
         eval_paraset, eval_paraset_attn, eval_paraset_len_mask) in evaluation_dataloader:
        with torch.no_grad():
        ...
        pos_idx = [idx for pos in eval_pos_seqs for idx, a in enumerate(pos) if pos[idx] == 1]
        target_label = [eval_labels[batch][pos_idx[batch]] for batch in range(eval_labels.size()[0])]

        # # predicted shape: (batch_size, 2)
        predicted, attn_weight = model(eval_pos_seqs, eval_text, eval_lengths,
                                        pos_idx, eval_paraset, eval_paraset_attn, eval_paraset_len_mask)



        attn_weight = attn_weight.squeeze(-1)
        _, attn_weight_max = torch.max(attn_weight, -1)
        attn_select_result = attn_weight_max - eval_para_idx
        tmp_arr = [1 for batch_idx in range(len(eval_pos_seqs)) if attn_select_result[batch_idx].data == 0]
        if len(tmp_arr) != 0:
            tmp_arr = np.mean(np.array(tmp_arr))
        else:
            tmp_arr = 0
        attn_acc.append(tmp_arr)

        target_label = torch.stack(target_label)
        batch_loss = criterion(predicted.view(-1, 2), target_label.view(-1))
        ...
        confusion_matrix = update_confusion_matrix(confusion_matrix, predicted_labels, target_label.data, eval_pos_seqs)
    attn_acc = np.array(attn_acc)
    attn_acc = np.mean(attn_acc)
    print("attention accuracy: ", attn_acc)
    ...
    return average_eval_loss, print_info_class(confusion_matrix), attn_acc
...
def update_confusion_matrix(matrix, predictions, labels, pos_seqs):
    ...
    :param matrix: a 3D numpy array of shape (2, 2)
...
def write_predictions(raw_dataset, evaluation_dataloader, model, using_GPU, rawdata_filename):
    ...
    for (eval_pos_seqs, eval_text, eval_lengths, eval_labels) in evaluation_dataloader:
        with torch.no_grad():
            eval_text = Variable(eval_text)
        mask = torch.ones(eval_labels.size()[0], eval_labels.size()[1])
        for temp_index in range(eval_labels.size()[0]):
            mask[temp_index, eval_lengths[temp_index].data:] = 0
        mask = mask.bool()
        with torch.no_grad():
        ...
        # get 0 or 1 predictions
        # predicted_labels: (batch_size, seq_len)
        predicted_labels = model.decode(predicted.data, mask=mask)
...
def print_info_class(grid):
    '''

    :param grid: a confusion matrix of shape(2, 2)
    :return:
    '''
    ...
    print('PRFA performance  ', precision, recall, f1, accuracy)
...
def print_info(matrix, idx2pos):
    ...
    for idx in range(len(idx2pos)):
        pos_tag = idx2pos[idx]
        grid = matrix[idx]
        precision = 100 * grid[0, 0] / np.sum(grid[0])
        recall = 100 * grid[0, 0] / np.sum(grid[:, 0])
        f1 = 2 * precision * recall / (precision + recall)
        print('PRFA performance for #tag0 ', pos_tag, precision, recall, f1)
        precision = 100 * grid[1, 1] / np.sum(grid[1])
        recall = 100 * grid[1, 1] / np.sum(grid[:, 1])
        f1 = 2 * precision * recall / (precision + recall)
        print('PRFA performance for #tag1 ', pos_tag, precision, recall, f1)
        accuracy = 100 * (grid[1, 1] + grid[0, 0] + grid[2,2]) / np.sum(grid)
        precision = 100 * grid[2, 2] / np.sum(grid[2])
        recall = 100 * grid[2, 2] / np.sum(grid[:, 2])
        f1 = 2 * precision * recall / (precision + recall)
        print('PRFA performance for #tag2 ', pos_tag, precision, recall, f1, accuracy)
...
class TextDatasetWithGloveElmoSuffix(Dataset):
    def __init__(self, embedded_text, pos_seqs, labels, paraphrase, para_sets):
        """
        ...
        :param paraphrase: paraphrase for the target word
        :param para_sets: paraphrase set of the target word
        """
    ...
    def __getitem__(self, idx):
        ...
        example_paraphrase = self.paraphrase[idx]
        example_para_sets = self.para_sets[idx]
        ...
        return example_pos_seq, example_text, example_length, example_label_seq, example_paraphrase, example_para_sets
    ...
    def collate_fn(batch):
        ...
        batch_pos_seqs = []
        batch_para = []
        batch_padded_paraset = []
        batch_padded_paraset_attn = []
        batch_padded_para_len_mask = []

        # Get the length of the longest sequence in the batch
        max_length = -1
        max_para_len = -1
        max_para_set_size = -1
        for pos, __, __, __, __, paraset in batch:
            if len(pos) > max_length:
                max_length = len(pos)
            for p in paraset:
                if p.size()[0] > max_para_len:
                    max_para_len = p.size()[0]
            if len(paraset) > max_para_set_size:
                max_para_set_size = len(paraset)

        # Iterate over each example in the batch
        for pos, text, length, label, para, paraset in batch:
            ...
            padded_para_set = []
            len_mask_array = []
            for para_candidate in paraset:

                # pad each paraphrase to the max length of the paraphrases in the whole batch
                can_para_len = para_candidate.size()[0]
                amount_to_pad = max_para_len - can_para_len
                pad_para = torch.zeros(amount_to_pad, para_candidate.shape[1])
                pad_candi_para = torch.cat((para_candidate, pad_para), dim=0)
                padded_para_set.append(pad_candi_para)

                # padding mask for the paraphrases
                len_mask_array.append(can_para_len)
            padded_para_set = torch.stack(padded_para_set)
            cur_para_size = len(paraset)
            amount_to_pad = max_para_set_size - cur_para_size
            attention_mask = [1] * cur_para_size + [0] * amount_to_pad
            pad_para_tensor = torch.zeros(amount_to_pad,padded_para_set[0].size()[0],padded_para_set[0].size()[1])
            padded_para_set = torch.cat((padded_para_set, pad_para_tensor), dim=0)

            len_mask_array = torch.LongTensor(len_mask_array)
            padded_len_mask_tensor = torch.zeros(amount_to_pad).long()
            padded_len_mask_array = torch.cat((len_mask_array, padded_len_mask_tensor), dim=0)
            ...
            batch_para.append(para)
            batch_padded_paraset.append(padded_para_set)
            batch_padded_paraset_attn.append(attention_mask)
            batch_padded_para_len_mask.append(padded_len_mask_array)
        ...
        return (batch_pos_seqs,
                ...
                torch.LongTensor(batch_para),
                torch.stack(batch_padded_paraset),
                torch.LongTensor(batch_padded_paraset_attn),
                torch.stack(batch_padded_para_len_mask))



def process_paraphrase(s):
    ''' Paraphrase extracted from the WordNet can not be used directly. Words such as puncts need to be seperated from
    words. This function is used to process the paraphrase.

    :param s: paraphrase extracted from WordNet
    :return: processed paraphrase
    '''
    front_space = [')', ';', '\'', ',']
    end_space = ['(']
    ...
    # print(s_list)
    while (idx < list_len):
        if (s_list[idx] in front_space and s_list[idx - 1] != ' '):
            s_list.insert(idx, ' ')
            idx = idx + 1
            list_len = list_len + 1
        elif (s_list[idx] in end_space and s_list[idx + 1] != ' '):
            s_list.insert(idx + 1, ' ')
            list_len = list_len + 1
        idx = idx + 1
    # print(s_list)
    paraphrase = ''.join(s_list)

    return paraphrase

def paraphrase_embed(paraphrase, word2idx, glove_embeddings):
    ''' Get the glove embeddings of each paraphrase.

    :param paraphrase: a word sequence for a paraphrase
    :param word2idx: a dictionary from word to index
    :return: the glove embeddings for paraphrase
    '''
...
def sequence_embed(sequence, word2idx, glove_embeddings):
    ''' Get the glove embeddings of each paraphrase.
    ...
    :return: the glove embeddings for paraphrase
    '''
    indexed_sequence = [word2idx.get(x, 1) for x in sequence]
    # glove_part has shape: (seq_len, glove_dim)
    glove_part = glove_embeddings(Variable(torch.LongTensor(indexed_sequence)))
    return glove_part