from util import get_num_lines, get_pos2idx_idx2pos, index_sequence, get_vocab, embed_indexed_sequence, \
    ...
    paraphrase_embed, sequence_embed
from util import TextDatasetWithGloveElmoSuffix as TextDataset
from util import evaluate
from model_rnn_hg import RNNSequenceModel, ExpModel, ParaEncoder, LinearFC, ModelSet
...
for i in range(10):
    ...
    # embedding_dim = elmo + suffix indicator
    # dropout1: dropout on input to RNN
    # Exp_model = ExpModel(embedding_dim=1024, hidden_size=256,
    #                                 num_layers=1, bidir=True,
    #                                 dropout1=0.5)
    Exp_model = ExpModel(embedding_dim=300, hidden_size=256,
                                    num_layers=1, bidir=True,
                                    dropout1=0.5)
    Query_model = ExpModel(embedding_dim=300, hidden_size=256,
                                    num_layers=1, bidir=True,
                                    dropout1=0.5)
    ...
    para_encoder = ParaEncoder(embedding_dim=300, hidden_size=256,
                         num_layers=1, attn_model=para_encoder_attn_model, bidir=True,
                         dropout1=0.5)
    linearfc = LinearFC(num_classes=2, encoded_embedding_dim=512, context_dim=512, dropout1=0.5)
    ...
    for epoch in range(num_epochs):
        ...
        for (example_pos, example_text, example_lengths, labels, para_idx, paraset, paraset_attn, paraset_len_mask) in train_dataloader_mohx:
            ...
            predicted, attn_weight = models(example_pos, example_text, example_lengths,
                                            pos_idx, paraset, paraset_attn, paraset_len_mask, para_idx)


            # attn_select = [-torch.log(attn_weight[batch][para_idx[batch]][0]) for batch in range(labels.size()[0])]
            # attn_select = torch.stack(attn_select)
            # attn_loss = torch.sum(attn_select)
            ...
            # batch_loss = batch_loss + attn_loss
            ...
            if num_iter % 50 == 0:
                ...
                # avg_eval_loss, performance_matrix = evaluate(idx2pos, train_dataloader_mohx, models,
                #                                              loss_criterion, using_GPU)
